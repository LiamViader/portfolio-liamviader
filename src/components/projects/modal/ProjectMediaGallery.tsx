...
const viewportWidthExpression = `calc(100vw - ${VIEWPORT_MEDIA_MARGIN_PX}px)`;
const viewportHeightExpression = `calc(100vh - ${VIEWPORT_MEDIA_MARGIN_PX}px)`;

const buildViewportFrameStyle = (aspectRatio?: number): CSSProperties => {
  const baseStyle: CSSProperties = {
    maxHeight: viewportHeightExpression,
    maxWidth: viewportWidthExpression,
  };

  if (!aspectRatio || Number.isNaN(aspectRatio) || aspectRatio <= 0) {
    return baseStyle;
  }

  const normalizedAspectRatio = aspectRatio;
  const heightMultiplier = Number((1 / normalizedAspectRatio).toFixed(6));

  return {
    ...baseStyle,
    width: `min(${viewportWidthExpression}, calc(${viewportHeightExpression} * ${normalizedAspectRatio}))`,
    height: `min(${viewportHeightExpression}, calc(${viewportWidthExpression} * ${heightMultiplier}))`,
  };
};

const getAspectRatioFromSize = (size: { width: number; height: number } | null) => {
...
export function ProjectMediaGallery({
  ...
  const viewportMediaStyle = useMemo(() => buildViewportFrameStyle(), []);
  const activeMediaAspectRatio = useMemo(
    () => getAspectRatioFromSize(intrinsicSize),
    [intrinsicSize],
  );
  const activeMediaFrameStyle = useMemo(
    () => buildViewportFrameStyle(activeMediaAspectRatio),
    [activeMediaAspectRatio],
  );

  const handleIntrinsicSizeUpdate = useCallback((width: number, height: number) => {
            ...
            {activeMedia && (
              ...
              >
                <button
                  ...
                  className="pointer-events-auto absolute right-4 top-4 z-[100101] inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/20 bg-slate-950/85 text-white shadow-[0_18px_48px_rgba(15,23,42,0.65)] backdrop-blur hover:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-sky-400 sm:right-6 sm:top-6"
                >
                    ...
                    {activeMedia.type === "image" && (
                      <img
                        ...
                        onLoad={(event) => {
                          ...
                          handleIntrinsicSizeUpdate(naturalWidth, naturalHeight);
                        }}
                      />
                    )}
                    {activeMedia.type === "video" && (
                      <video
                        ...
                        onLoadedMetadata={(event) => {
                          ...
                          handleIntrinsicSizeUpdate(videoWidth, videoHeight);
                        }}
                      />
                    )}
